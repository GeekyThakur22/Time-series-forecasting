...
--4.Calculate Monthly Sales Growth:
...
7.Calculate Weekly Sales Trends:
...
--8.Identify Seasonal Patterns using Moving Averages:
SELECT [Order Date],
       [Sales],
...
--9.Calculate Cumulative Sales Over Time:
SELECT [Order Date], 
       SUM([Sales]) OVER (ORDER BY [Order Date]) AS CumulativeSales
FROM [dbo].['Timeseries_data(1)$']
ORDER BY [Order Date];

--10.Identify Days with Abnormally High/Low Sales:
SELECT [Order Date], 
       [Sales],
       CASE 
...
--13.Detect Outliers with Z-Score:
SELECT [Order Date],
       [Sales],
